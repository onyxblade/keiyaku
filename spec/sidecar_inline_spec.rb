...
# examples/sidecar-inline.json is not written by hand and not written for this:
# it is what `@fastify/swagger` prints for a running service, checked in exactly
# as the service emitted it. Everything the generator has to cope with in a
# document nobody wrote is true of it at once — no operationId, no components,
# no servers — and the two hand-written examples cannot stand in for that,
# because they were written by somebody who knew what the generator does with
# them.
#
# The service has since named its shared shapes, and examples/sidecar.json is
# what it prints now; sidecar_spec.rb covers that. This copy stays because what
# it holds is not the sidecar but the shape of a document with no components at
# all, which is a thing servers emit and which nothing else here exercises. It
# is frozen on purpose: it does not track the service, and refreshing it from
# the service would defeat the point of keeping it.
#
...
RSpec.describe "a document that names nothing itself" do
  subject(:operations) { SidecarInline::Client.operations }
  ...
  it "names every type after the operation it was written under" do
    operations.each do |name, op|
      expected = "SidecarInline::#{name.to_s.split("_").map(&:capitalize).join}"
  ...
  it "does not hand an operation its result type as its error type" do
    ...
    expect(op[:errors].values.uniq).to eq [SidecarInline::PostDidResolveError]
  ...
  it "leaves the address to the application" do
    expect(SidecarInline::Client.server).to be_nil
    expect { SidecarInline::Client.new }.to raise_error(Keiyaku::Error, /has no server declared/)
    expect(SidecarInline::Client.new(base_url: "http://127.0.0.1:3100").base_url).to eq "http://127.0.0.1:3100"
  end
  ...
  it "types what it cannot translate as :any, and only that" do
    options = SidecarInline::PostDidcommPackEncryptedBodyOptions
    attachments = SidecarInline::PostDidcommPackEncryptedBodyMessageAttachments
  ...
  it "keeps a property named like one of the model's own options" do
    message = SidecarInline::PostDidcommPackEncryptedBodyMessage
