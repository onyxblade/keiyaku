...
module Widgets
  class Client < Keiyaku::Client
    ...
    put    :replace_labels, "/widgets/{id}/labels", header: { "X-Tags" => :x_tags, "X-Context" => :x_context }, explode: %w[X-Context], body: { String => String }, body_required: true, into: { String => String }, errors: { 409 => [Problem] }
    get    :search_widgets, "/widgets/search", query: %i[q cursor filter], deep_object: %w[filter], required: %i[q], into: SearchWidgetsResult
    get    :list_events, "/widgets/{id}/events", query: %i[limit offset], into: [Event]
  end
end
